...
def get_ode_solver(cfg):
    ...
    elif name == 'SE3_Streaming_Euler':
        solver = SE3_Streaming_Euler(**cfg)
    elif name == 'SE3_Streaming_RK_mk':
        solver = SE3_Streaming_RK4_MK(**cfg)
    else:
...
class SE3_Streaming_Euler:

    def __init__(self, num_steps, output_interval=1):
        self.t = torch.linspace(0, 1, num_steps + 1)
        self.output_interval = output_interval

    @torch.no_grad()
    def __call__(self, z, x_0, func, streaming=False):
        if streaming:
            return self._streaming_integrate(z, x_0, func)
        else:
            return self._batch_integrate(z, x_0, func)

    def _batch_integrate(self, z, x_0, func):
        """Batch integration mode (backward compatible)"""
        t = self.t.to(z.device)
        dt = t[1:] - t[:-1]
        traj = x_0.new_zeros(x_0.shape[0:1] + t.shape + x_0.shape[1:])
        traj[:, 0] = x_0
    ...
    def _streaming_integrate(self, z, x_0, func):
        """Streaming integration mode - yields intermediate results"""
        t = self.t.to(z.device)
        dt = t[1:] - t[:-1]
        x_n = x_0.clone()
        ...
        for n in range(len(t)-1):
            ...
            x_next = deepcopy(x_n)
            x_next[:, :3, :3] @= exp_so3(h.unsqueeze(-1) * w_1)
            x_next[:, :3, 3] += h * v_1
            x_n = x_next

            # Yield at specified intervals
            if (n + 1) % self.output_interval == 0:
                yield x_n

        # Always yield final state
        if (len(t) - 1) % self.output_interval != 0:
            yield x_n


class SE3_Streaming_RK4_MK:
    ...
    def _streaming_integrate(self, z, x_0, func):
        ...
        for n in range(len(t)-1):
            ...
            x_next = deepcopy(x_n)
            x_next[:, :3, :3] @= exp_so3(u)
            x_next[:, :3, 3] += (h / 6) * (v_1 + 2 * v_2 + 2 * v_3 + v_4)
            x_n = x_next
